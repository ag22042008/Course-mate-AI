...
## How it works
`create_db.py` loads a PDF, splits it into overlapping chunks, embeds each chunk with Mistral's embedding model, and stores the vectors in a local Chroma database.

`streamlit_app.py` retrieves the most relevant chunks for a question using MMR (Maximal Marginal Relevance) search, stuffs them into a prompt as context, and asks the Mistral LLM to answer using only that context.

## Architecture

```mermaid
flowchart TD
    subgraph Indexing["Indexing — create_db.py (run once per document)"]
        A[PDF file] --> B[PyPDFLoader]
        B --> C[Split into overlapping chunks]
        C --> D[Mistral embeddings]
        D --> E[(Chroma DB<br/>chroma_db/)]
    end

    subgraph Query["Query — streamlit_app.py"]
        F[User question] --> G{Input mode}
        G -->|Typed| H[Chat box]
        G -->|Spoken| I[🎤 Voice recorder]
        I --> J[SpeechRecognition<br/>Google Web Speech API]
        J --> H
        H --> K[MMR search<br/>k / fetch_k / lambda_mult]
        E -.retrieve relevant chunks.-> K
        K --> L[Stuff chunks into prompt as context]
        L --> M[Mistral LLM]
        M --> N[Answer + catalog drawer<br/>source passages shown]
        N -->|if enabled| O[🔊 gTTS<br/>read answer aloud]
    end
```
...
- **Indexing (offline, one-time):** PDF → chunks → embeddings → Chroma DB on disk.
- **Query (per question):** question (typed or transcribed from voice) → MMR retrieval against Chroma DB → context + question → Mistral LLM → grounded answer, optionally spoken back via TTS.
...
```
Get a key from the Mistral AI console.
...
This uses:
- **SpeechRecognition** with the free Google Web Speech API for transcription (no API key needed)
- **gTTS** (Google Text-to-Speech) to synthesize the spoken answer
